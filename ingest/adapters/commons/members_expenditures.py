"""Stage 1 adapter for House of Commons Members expenditure reports.

Raw input is a pickle mapping report URLs to CSV bytes:

    https://www.ourcommons.ca/ProactiveDisclosure/en/members/{type}/{year}/{quarter}/{uuid}/csv

where {type} is contract | travel | hospitality. Each CSV has a quoted title
row ("Members – Detailed ... Report – {Last, First} – Q{n} {year}"), a header
row, then data rows. Travel files interleave two row kinds per claim group:
a cost-summary row (empty Traveller Name) followed by trip-segment rows.

Emits five row classes (see source/commons_members_expenditures.schema.yaml),
each tagged with "_row_class".
"""
...
from ingest.schema import load_source_schema, normalized_fields, publisher_field_map
