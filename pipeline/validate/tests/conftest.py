...
@pytest.fixture
def universe_paths(tmp_path):
    return UniversePaths(root=tmp_path / "universe")

@pytest.fixture
def write_staging_dir(universe_paths):
    def _write(source, staging_run_id, records_jsonl, **kwargs):
        return write_staging(universe_paths, source, staging_run_id, records_jsonl, **kwargs)
    return _write
...
def make_validate_context(
    ...
    *,
    ...
    return ValidateContext(
        ...
        paths=UniversePaths(root=tmp_path / "universe"),
        fail_fast=fail_fast,
    )
